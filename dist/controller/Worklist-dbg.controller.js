sap.ui.define([
	...
	return BaseController.extend("wel.CreatePO.controller.Worklist", {
		...
		callModels: function () {
			...
			// 	success: function (data, response) {
			// 		var PartnerFunctionModel = new JSONModel(data);
			// 		_self.setModel(PartnerFunctionModel, "PartnerFunctionModelName");
			// 	},
			// 	error: function () {}
			// });
				...
				success: function (data, response) {
					if (data.results.length) {
						...
						if (!((data.results[0].VendorPartnerNav.results).length)) {
							MessageBox.alert("No Partner/Partners available for" + " " + "(" + selectedVendorId + ")");
						} else {
							checkPartner = [];
						...
						MessageBox.alert("No Vendor available for" + " " + "(" + selectedVendorId + ")");
		...
		ounSubmit: function (oEvent) {
			...
			if (oEvent.getParameter("selectedItem")) {
				...
				this.getOwnerComponent().getModel().read("/OrderUnitOfMeasureSet", {
					...
					success: function (data, response) {
						if (!(data.results.length)) {
							MessageBox.alert("No Order Unit Of Measure available for" + " " + "(" + selectedOun + ")");
		...
		mtrlGrpSelection: function (oEvent) {
			...
			if (oEvent.getParameter("selectedItem")) {
				...
				this.getOwnerComponent().getModel().read("/MaterialGroupSet", {
					...
					success: function (data, response) {
						if (!(data.results.length)) {
							MessageBox.alert("No Material Group available for" + " " + "(" + selectedMaterialGroup + ")");
							oInput.setValueState("Error");
						} else {
							oInput.setValueState("None");
		...
		glAccSubmit: function (oEvent) {
			...
			if (oEvent.getParameter("selectedItem")) {
				...
				this.getOwnerComponent().getModel().read("/GLAccountSet", {
					...
					success: function (data, response) {
						if (!(data.results.length)) {
							MessageBox.alert("No GLAccount available for" + " " + "(" + selectedGlAcc + ")");
							oInput.setValueState("Error");
						} else {
							oInput.setValueState("None");
		...
		cstCntreSubmit: function (oEvent) {
			...
			if (oEvent.getParameter("selectedItem")) {
				...
				this.getOwnerComponent().getModel().read("/CostCenterSet", {
					...
					success: function (data, response) {
						if (!(data.results.length)) {
							MessageBox.alert("No CostCenter available for" + " " + "(" + selectedCostCenter + ")");
							oInput.setValueState("Error");
						} else {
							oInput.setValueState("None");
		...
		ordrSubmit: function (oEvent) {
			...
			if (oEvent.getParameter("selectedItem")) {
				...
				this.getOwnerComponent().getModel().read("/OrderNumberRefSet", {
					...
					success: function (data, response) {
						if (!(data.results.length)) {
							MessageBox.alert("No Order Number available for" + " " + "(" + selectedOreder + ")");
							oInput.setValueState("Error");
						} else {
							oInput.setValueState("None");
		...
		gbsSubmit: function (oEvent) {
			...
			if (oEvent.getParameter("selectedItem")) {
				...
				this.getOwnerComponent().getModel().read("/WBSElementSet", {
					...
					success: function (data, response) {
						if (!(data.results.length)) {
							MessageBox.alert("No WBS Element available for" + " " + "(" + selectedWBS + ")");
							oInput.setValueState("Error");
						} else {
							oInput.setValueState("None");
		...
		onAddItem: function (oEvent) {
			...
			var inputs = [
				// oView.byId("idItem"),
				oView.byId("idAccAGnmnt"),
				oView.byId("idShrtTxt"),
				oView.byId("idPoQty"),
				oView.byId("idOun"),
				oView.byId("idNetPrice"),
				oView.byId("idDelvDt"),
				oView.byId("idMtrlGrp"),
				oView.byId("idGlAcc"),
				oView.byId("idCstCntre"),
				oView.byId("idOrdr"),
				oView.byId("idGbs")
			];
				...
				if (this.mandtFldValidationCheck()) {
					...
					if (oView.byId("idItemsTable").getItems().length > 0) {
						...
						for (var iLcv = 0; iLcv < itemTableExistingModel.getData().results.length; iLcv++) {
							poNetValueCount = poNetValueCount + parseFloat(itemTableExistingModel.getData().results[iLcv].NetPrice);
						}
						...
						for (var iLcv2 = 0; iLcv2 < itemTableItem.results.length; iLcv2++) {
							poNetValueCount = (poNetValueCount + parseFloat(itemTableItem.results[iLcv2].NetPrice));
						}
					...
					// 	oView.byId("idSubmit").setEnabled(true);
					// }
		...
		onPersonalSettingsPress: function (oEvent) {
			...
			// } else {
			// 	oView.byId("idPurchOrg").setSelectedKey("");
			// 	oView.byId("idPurchGrp").setSelectedKey("");
			// 	oView.byId("idCurrency").setSelectedKey("");
			// 	oView.byId("idCompCode").setSelectedKey("");
			// }
		},
		...
		itemTableSelectionChange: function (oEvent) {
			...
			oView.byId("idDelvDt").setValue(this.formattingDate(selectedRow.DelivDate));
			...
			if (!canBeProceed) {
				...
				if (this.mandtFldValidationCheck()) {
					...
					for (var npuLcv = 0; npuLcv < (this.getView().byId("idItemsTable").getModel("itemTableModelName").getData().results).length; npuLcv++) {
						poNetValue = poNetValue + parseFloat(this.getView().byId("idItemsTable").getModel("itemTableModelName").getData().results[npuLcv]
							.NetPrice)
					}
			...
			if (this.getView().byId("idItemsTable").getItems().length) {
				...
				if (itemsHavetoCopy.length) {
					...
					for (var ciLcv3 = 0; ciLcv3 < itemTable.results.length; ciLcv3++) {
						totNetPrice = totNetPrice + parseInt(itemTable.results[ciLcv3].NetPrice);
					}
					...
					MessageBox.alert("Please select item from the table.");
				}
			} else {
				MessageBox.alert("Please add item to the table.");
			}
		},
		onDeleteItem: function (oEvent) {
				...
				if (colDelItems.length) {
					for (var sILcv2 = (itemTableData.length - 1); sILcv2 >= 0; sILcv2--) {
						...
						if (itemTableData[sILcv2].selected) {
							totPoNetValue = parseFloat(totPoNetValue) - parseFloat(itemTableData[sILcv2].NetPrice);
							currency = (itemTableData[sILcv2].NetPrice).split(" ")[1];
							itemTableData.splice(sILcv2, 1);
						}
					}
					fnlNetValue = parseFloat(totPoNetValue) + " " + currency;
					...
					MessageBox.alert("Please select record from the table.");
				}
			} else {
				MessageBox.alert("Please add record to the table.");
			}
			// var itemTableLength = oEvent.getSource().getParent().getParent().mAggregations.items
			// if (itemTableLength.length) {
			// 	var itemTableFinalRecords = this.getView().byId("idItemsTable").getModel("itemTableModelName").getData();
			// 	var checkSelectAll = oEvent.getSource().getParent().getParent().mAggregations.columns[0].getHeader().getSelected();
			// 	if (checkSelectAll) {
			// 		for (var sILcv = 0; sILcv < itemTableLength.length; sILcv++) {
			// 			itemTableFinalRecords.results.splice(0, 1);
			// 		}
			// 		oEvent.getSource().getParent().getParent().mAggregations.columns[0].getHeader().setSelected(false);
			// 	} else {
			// 		for (var sILcv2 = (itemTableLength.length - 1); sILcv2 >= 0; sILcv2--) {
			// 			if (itemTableLength[sILcv2].getCells()[0].getSelected()) {
			// 				itemTableFinalRecords.results.splice(sILcv2, 1);
			// 				itemTableLength[sILcv2].mAggregations.cells[0].setSelected(false);
			// 			} else {
			// 				checkedItems = false;
			// 			}
			// 			// itemTableLength[sILcv2].mAggregations.cells[0].setSelected(false);
			...
			// 	}
			// 	if (!checkedItems) {
			// 		MessageBox.alert(resourceBundle.getText("plsSelectRcrdFrmPrtnrTbl"));
			// 	}
			// 	// if (!(itemTableFinalRecords.results.length)) {
			// 	// 	this.getView().byId("idSubmit").setEnabled(false);
			// 	// }
			// 	var itemTableModel = new JSONModel(itemTableFinalRecords);
			// 	this.setModel(itemTableModel, "itemTableModelName");
			// } else {
			// 	MessageBox.alert(resourceBundle.getText("plsSelectRcrdFrmPrtnrTbl"));
			// }
		...
		onAddPartner: function (oEvent) {
			...
			if (!canProceed2) {
				...
				if (this.addPartnerMandtFldChk()) {
					...
					MessageBox.alert("Please fillup mandatory fields.");
				}
				...
				this.getOwnerComponent().getModel().read("/PartnerFunctionCheckSet", {
					...
					success: function (data, response) {
						if (data.results[0].Vtext === "1") {
							...
							MessageBox.error(resourceBundle.getText("prtnr") + " " + data.results[0].Parvw + " " + resourceBundle.getText("and") + " " +
								resourceBundle.getText("vndr") + " " + data.results[0].Lifnr + " " +
								resourceBundle.getText("arMismatchd"));
							oView.byId("idAddNewItem").setVisible(false);
							// oView.byId("idAddPartner").setVisible(false);
						}
					},
		...
		selectPartnerSingleRow: function (oEvent) {
			...
			// var oTable = this.getView().byId("idPartnersTable");
			...
			// 	var model = oTable.getModel("vendorPartnerModelName");

			// 	var array = this.getSelectedIndices();

			// 	if (array.length > 0) {

			// 		model.getData()['vendorPartnerModelName'][rowIndex]['checked'] = true;

			// 	} else {

			// 		model.getData()['vendorPartnerModelName'][rowIndex]['checked'] = false;
		...
		onDeletePartner: function (oEvent) {
			...
			// var resourceBundle = this.getResourceBundle();
			// var partnerTableLength = oEvent.getSource().getParent().getParent().mAggregations.items;
			// var checkedOrNot = true;
			// if (partnerTableLength.length) {
			// 	var partnerTableFinalRecords = this.getView().byId("idPartnersTable").getModel("vendorPartnerModelName").getData();
			// 	var checkSelectAll = oEvent.getSource().getParent().getParent().mAggregations.columns[0].getHeader().getSelected();
			// 	if (checkSelectAll) {
			// 		for (var sILcv = 0; sILcv < partnerTableLength.length; sILcv++) {
			// 			partnerTableFinalRecords.results.splice(0, 1);
			// 		}
			// 		oEvent.getSource().getParent().getParent().mAggregations.columns[0].getHeader().setSelected(false);
			// 	} else {
			// 		for (var sILcv2 = (partnerTableLength.length - 1); sILcv2 >= 0; sILcv2--) {
			// 			if (partnerTableLength[sILcv2].getCells()[0].getSelected()) {
			// 				partnerTableFinalRecords.results.splice(sILcv2, 1);
			// 				partnerTableLength[sILcv2].mAggregations.cells[0].setSelected(false);
			// 			} else {
			// 				checkedOrNot = false;
			// 			}
			// 			// itemTableLength[sILcv2].mAggregations.cells[0].setSelected(false);
			...
			// 		MessageBox.alert(resourceBundle.getText("plsSelectRcrdFrmPrtnrTbl"));
			// 	}
			// 	// if (!(partnerTableFinalRecords.results.length)) {
			// 	// 	this.getView().byId("idSubmit").setEnabled(false);
			// 	// }
			// 	var itemTableModel = new JSONModel(partnerTableFinalRecords);
			// 	this.setModel(itemTableModel, "vendorPartnerModelName");
			// } else {
			// 	MessageBox.alert(resourceBundle.getText("plsSelectRcrdFrmPrtnrTbl"));
			// }
		},
		AccAssgnmntSelectionChange: function (oEvent) {
			...
			if (oEvent.getSource().getSelectedItem().getKey() === "F") {
				...
				if (oView.byId("idGbs").getValue()) {
					oView.byId("idGbs").setValue("");
					// oView.byId("idGbs").getSelectedItem().setKey("");
				}
				oView.byId("idOrdLbl").setVisible(true);
				oView.byId("idOrdr").setVisible(true);
				oView.byId("idCstCntreLbl").setVisible(true);
				...
				if (oView.byId("idOrdr").getValue()) {
					oView.byId("idOrdr").setValue("");
					// oView.byId("idOrdr").getSelectedItem().setKey("");
				}
				oView.byId("idGbsLbl").setVisible(false);
				oView.byId("idGbs").setVisible(false);
				if (oView.byId("idGbs").getValue()) {
				...
				if (oView.byId("idCstCntre").getValue()) {
					oView.byId("idCstCntre").setValue("");
					// oView.byId("idCstCntre").getSelectedItem().setKey("");
				}
				oView.byId("idGbsLbl").setVisible(true);
				oView.byId("idGbs").setVisible(true);
			}
		...
		onPersonalSettingsDialogAdd: function (oEvent) {
			...
			if (!canContinue) {
				MessageBox.alert("Please fill up mandatory fields.");
			} else {
				// if (this.getView().byId("idVendor").getValue()) {
				var finalItems = [];
				...
				var WAERS = oView.byId("idCurrency").getSelectedItem().getKey();
				...
				var currency = {
					"Element": "POHeaderProposer    WAERS",
					"Active": WAERS
				};
				finalItems.push(currency);
				var compCode = {
				...
				oModel.create("/PersonalSettingsHeadSet", payLoad, {
					success: function (data, response) {
						...
						if (_self.getView().byId("idItemsTable").getItems()) {
							for (var dcLcv = 0; dcLcv < _self.getView().byId("idItemsTable").getItems().length; dcLcv++) {
								var price = _self.getView().byId("idItemsTable").getItems()[dcLcv].mAggregations.cells[6].getText().split(" ")[0];
								for (var dcLcv2 = 0; dcLcv2 < data.PerSetHdToItmNav.results.length; dcLcv2++) {
									if ((data.PerSetHdToItmNav.results[dcLcv2].Element).includes("WAERS")) {
										var currPrice = price + " " + data.PerSetHdToItmNav.results[dcLcv2].Active;
										_self.getView().byId("idItemsTable").getItems()[dcLcv].mAggregations.cells[6].setText(currPrice);

									}
								}

							}
							var poNetValueFinal = parseFloat(_self.getView().byId("idPoNetValue").getText()) + " " + data.PerSetHdToItmNav.results[2].Active;
							_self.getView().byId("idPoNetValue").setText(poNetValueFinal);
						}
					},
					error: function (response) {
						sap.ui.core.BusyIndicator.hide();
				...
				// } else {
				// 	MessageBox.alert("Please select vendor.");
				// }
			}
		},
		...
		formatDate: function (dateValue) {
			var selDate = new Date(dateValue);
		...
		// onFunctnValueHelpRequest: function(){
		...
		// onFunctnSearch: function (oEvent) {
		// 	var oFilter = "";
		// 	var sQuery = oEvent.getParameter("value");
		// 	if (sQuery && sQuery.length > 0) {
		// 		oFilter = new Filter({
		...
		onFunctnSelection: function (oEvent) {
			...
			if (oEvent.getParameter("selectedItem")) {
				...
				this.getOwnerComponent().getModel().read("/PartnerFunctionSet", {
					...
					success: function (data, response) {
						if (!(data.results.length)) {
							MessageBox.alert("No Function available for" + " " + "(" + selectedFunctn + ")");
						}
						oInput.setValueState("None");
						oInput2.setValueState("None");
			...
			if (selectedVendor) {
				if (documentDate) {
					if (itemTable.getItems().length) {
						...
						// else{
						// 	MessageBox.alert("Please add record to the partner table.");
						// }
						...
						if (itemTable.getItems.length) {
							for (var itmLcv = 0; itmLcv < itemTableRecords.length; itmLcv++) {
								// if (itemTableRecords[itmLcv].selected) {
								HeadToItemNavObj.Ebelp = itemTableRecords[itmLcv].Item;
						...
						// if (partnerItemSelected) {
						// if (itemTableItemSelected || partnerItemSelected) {
						sap.ui.core.BusyIndicator.show();
						...
						// } else {
						// 	MessageBox.alert(oResourceBundle.getText("itmRecSelMsg"));
						// }
						// }
						// else {
						// 	MessageBox.alert(oResourceBundle.getText("prtnrRcrdSelMsg"));
						// }
						// }else{
						// 	MessageBox.alert("Please add record to the partner table.");
						// }
					} else {
						MessageBox.alert("Please add record to the item table.");
					}
